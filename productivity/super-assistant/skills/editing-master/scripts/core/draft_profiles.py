...
TEMPLATE_ROOT = Path(__file__).resolve().parents[1] / "engine" / "templates"


PROFILES: Dict[str, DraftProfile] = {
    "capcut_legacy": DraftProfile(
        name="capcut_legacy",
        template_dir=str(TEMPLATE_ROOT / "capcut_legacy"),
    ...
    "jianying_legacy": DraftProfile(
        ...
        template_dir=str(TEMPLATE_ROOT / "jianying_legacy"),
        content_file="draft_info.json",
        is_capcut_env=False,
    ...
    "jianying_pro_10": DraftProfile(
        ...
        template_dir=str(TEMPLATE_ROOT / "jianying_pro_10"),
...
def get_draft_profile(name: Optional[str] = None) -> DraftProfile:
    if name is None:
        try:
            from core.settings.local import DRAFT_PROFILE
        except Exception:
