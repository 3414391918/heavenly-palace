...
- **Agent 工作知识**只保存会改变 Agent 决策的稳定架构边界、研发规范、环境入口、平台操作和定位路由。正文应短、可检索、指向权威来源；默认可由上层 Skill 按任务加载。
- **人读排障知识**保存具体问题、现象、排查证据、根因、修复、验证和决策，帮助人理解项目。保留完整过程，但 Agent 只在用户询问历史、遇到相似故障或需要复用排查路径时检索，不能默认注入每次研发任务。
- **功能设计与演进**记录具体能力的方案和变化，只在任务命中该功能时读取，不自动升级为全项目规范。

项目目录可按这三类建立子目录；已有明确分类时沿用。知识导航与上层 Skill 要路由到已确认的 Agent 工作知识，不把待审查页、排障流水或功能史当默认上下文。
