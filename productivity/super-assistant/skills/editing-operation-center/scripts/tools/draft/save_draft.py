...
import shutil
from core.util import build_draft_asset_path
from typing import Dict, Literal
from core.draft_cache import DRAFT_CACHE
from core.save_task_cache import get_task_status, update_tasks_cache, update_task_field, update_task_fields, create_task
...
import logging
import requests
from core.draft_profiles import get_draft_profile, write_profile_content
...
def save_draft_background(draft_id, draft_folder, task_id):
    """Materialize a draft and its media files in a local folder."""
    try:
        ...
        if download_tasks:
            ...
            with ThreadPoolExecutor(max_workers=16) as executor:
                ...
                for future in as_completed(future_to_task):
                    ...
                    try:
                        ...
                        if not local_path:
                            raise RuntimeError(
                                f"Downloader returned failure for {task['args'][0]}"
                            )
                        ...
                        download_errors.append(f"{task['type']}: {e}")
            ...
            if download_errors:
                raise RuntimeError(
                    "Failed to download required media: " + "; ".join(download_errors)
                )
        ...
        update_task_field(task_id, "draft_path", os.path.abspath(draft_dir))
        logger.info(f"Task {task_id} completed, draft path: {draft_dir}")
        return {
            "draft_id": draft_id,
...
def save_draft_impl(draft_id: str, draft_folder: str = None) -> Dict[str, str]:
    """Save a draft synchronously to a local JianYing/CapCut project folder."""
    ...
    try:
        ...
        result = save_draft_background(draft_id, draft_folder, task_id)
